...
def testFunction() -> None:
    col = mw.col
    transfer_scheduling_data(col, 'Hanzi', 'Domino Text Input-43bf8', 'Recall', 'Domino Recognition and Stroke Order-6c462', 'Recall')
...
def transfer_scheduling_data(col: Collection, field_name: str, source_card_type_name: str, source_card_template_name: str, target_card_type_name: str, target_card_template_name: str):
    """
    ...
    source_card_ids = col.find_cards(f"\"note:{source_card_type_name}\" card:{source_ord+1}")
    target_card_ids = col.find_cards(f"\"note:{target_card_type_name}\" card:{target_ord+1}")
    ...
    for card_id in target_card_ids:
        ...
        note = card.note()
        field_value = note.fields[col.models.field_map(note.note_type())[field_name][0]]
    ...
    for source_card_id in source_card_ids:
        source_card = col.get_card(source_card_id)
        source_note = source_card.note()
        source_field_value = source_note.fields[col.models.field_map(source_note.note_type())[field_name][0]]

        if source_field_value in field_to_target_card_ids:
            
            target_card_id = field_to_target_card_ids[source_field_value]
            target_card = col.get_card(target_card_id)
            ...
            col.update_card(target_card)

            print(f"Replace scheduling data from card {source_card_id} (due {duedate_after}) to card {target_card_id} (due {duedate_before})")

            col.db.execute("delete from notes where id = ?", source_card.nid)
            col.db.execute("delete from cards where id = ?", source_card_id)

            col.db.execute("update cards set id = ? where id = ?", source_card_id, target_card_id)
            col.db.execute("update notes set usn = -1 where id = ?", target_card.nid)

    epoch = str(time.time()).replace(".", "")[0:13]
    col.db.execute("update col set mod = ?", epoch) 
    col.save()

def print_card_types(col: Collection):
    """
...
def print_template_name_of_each_card(col):
    """
    ...
    for model in col.models.all():
        ...
        card_ids = col.find_cards(f"mid:{model_id}")
...
def get_card_due_date(col: Collection, card_id: int) -> int:
    """
...
if __name__ == '__main__':
    ...
    col = Collection(r'C:\Users\Thaler\AppData\Roaming\Anki2\Usuário 1\collection.anki2')
    # print_card_types(col)
    # print_template_name_of_each_card(col)
